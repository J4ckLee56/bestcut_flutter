...
class AIService {
  ...
  Future<void> recognizeSpeech() async {
    ...
    try {
      ...
      if (segments.isNotEmpty) {
        ...
        // 음성인식 완료 후 자동으로 내용 요약 시작
        print('음성인식 완료. 자동으로 내용 요약 시작...');
        await summarizeScript();
      } else {
  ...
  Future<void> summarizeScript() async {
    ...
    try {
      ...
      for (int i = 0; i < chunks.length; i++) {
        ...
        final overview = await _getChunkOverview(chunks[i], i + 1, chunks.length, apiKey, uri);
      ...
      final overallStructure = await _integrateChunkOverviews(chunkOverviews, apiKey, uri);
      ...
      for (int i = 0; i < themeGroups.length; i++) {
        ...
        final selectedIds = await _summarizeThemeGroup(group, apiKey, uri);
      ...
      // 최종 요약 텍스트 생성
      final finalSummary = await _generateFinalSummary(uniqueSelectedIds, apiKey, uri);
  ...
  Future<List<WhisperSegment>> callLocalWhisper(String audioPath) async {
    ...
    try {
      ...
      if (result.exitCode != 0) {
        throw StateError('whisper.cpp 실행 실패: ${result.stderr}');
      }
...
''';
  ...
  // 영상 개요 분석 (AI API 사용)
  Future<Map<String, dynamic>> getVideoOverview(List<WhisperSegment> segments, String apiKey, Uri uri) async {
    final formatted = segments.map((s) => {
      'id': s.id,
      'start': s.startSec,
      'end': s.endSec,
...
''';
  ...
  String _getAppResourcesPath() {
    // 개발 중인 경우 프로젝트 경로 사용
    if (kDebugMode) {
      return '/Users/ihuijae/Desktop/Flutter_Workspace/bestcut_flutter/build/macos/Build/Products/Debug/bestcut_flutter.app/Contents/Resources';
    }
    
    // 배포된 앱의 경우 실행 파일 기준 상대 경로
